...
class D3QNPERAgent():
    ...
    def __init__(self, gamma, epsilon, lr, input_dims, batch_size, n_actions,
        ...
        self.memory = PrioritizedReplayBuffer(max_size=max_mem_size, obs_dim=input_dims)
    ...
    def learn(self):
        ...
        states, actions, rewards, states_, dones, tree_idxs, IS_weights =  self.memory.sample_batch(self.batch_size)
        ...
        losses = F.smooth_l1_loss(q_pred, expected_q_values, reduction='none')

        #compute TD Error
        td_errors = T.abs(q_pred - expected_q_values).detach().cpu().numpy()

        self.memory.update_priorities(tree_idxs, td_errors)

        IS_weights_tensor = T.as_tensor(IS_weights, dtype=T.float32).to(device)
        loss_mean = (losses * IS_weights_tensor).mean()
        ...
        # Mean max Q-value over the batch (proxy for value estimate health)
        with T.no_grad():
        ...
        return loss_mean.item(), grad_norm, q_mean