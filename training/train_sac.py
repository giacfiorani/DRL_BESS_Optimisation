...
DEFAULT_WARMUP_STEPS = 5000  # random actions before learning begins
...
def evaluate_on_val(agent, val_env):
    """Run a deterministic rollout over the entire validation split.

    The actor is placed in eval mode and ``choose_action_deterministic`` is
    used. No transitions are stored, preventing replay buffer contamination.
...
def train(
    ...
    """Train the SAC agent on the BESS environment.

    Args:
        agent_name: Display name for the agent (used in run naming).
        ...
        warmup_steps: Number of random-action steps before SAC begins learning.
    ...
    for i in range(hp["n_episodes"]):
        ...
        while not done:
            ...
            # Truncated episodes (dataset exhaustion) should still bootstrap the
            # value target. Using `done` here would introduce a ~3.4% downward
            # value bias but would prevent unbounded Q-drift from function
            # approximation errors compounding across episode boundaries.
            agent.store_transition(obs, action, reward, obs_, terminated)

            # SAC updates every step, unlike the episodic discrete agents.
            ...
            # Only append losses if the agent has exited warmup.
